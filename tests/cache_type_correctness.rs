...
use turboquant::cache::{PqoCache, TqCache};
...
fn make_kv(seq_len: usize, seed: f32) -> (Tensor, Tensor) {
    ...
    let k: Vec<f32> = (0..n).map(|i| ((i as f32 + seed) * 0.0137).sin() * 2.0).collect();
    let v: Vec<f32> = (0..n).map(|i| ((i as f32 + seed + 1000.0) * 0.0213).cos() * 1.5).collect();
    (
...
fn make_q(seq_len: usize) -> Tensor {
    Tensor::zeros((1, NUM_KV_HEADS * 2, seq_len, HEAD_DIM), DType::F32, &Device::Cpu).unwrap()
}
...
fn pq3_uses_standard_codebook() {
    let mut cache = PqoCache::with_outlier_blocks(3, HEAD_DIM, NUM_KV_HEADS, NUM_LAYERS, QuantNormMode::MaxNorm, 0);
...
fn pq3_quality_lower_than_pqo3() {
    ...
    let mut pq = PqoCache::with_outlier_blocks(3, HEAD_DIM, NUM_KV_HEADS, NUM_LAYERS, QuantNormMode::MaxNorm, 0);
    let mut pqo = PqoCache::new(3, HEAD_DIM, NUM_KV_HEADS, NUM_LAYERS, QuantNormMode::MaxNorm);
...
fn pqo3_uses_outlier_codebook() {
    let mut cache = PqoCache::new(3, HEAD_DIM, NUM_KV_HEADS, NUM_LAYERS, QuantNormMode::MaxNorm);
    let (k, v) = make_kv(4, 2.0);
    let q = make_q(4);
    let result = cache.prefill(LAYER, &k, &v, &q).unwrap();
    assert!(result.logit_bias.is_none(), "PQO3 should have no logit_bias");
}

#[test]
fn pqo4_uses_outlier_codebook() {
    let mut cache = PqoCache::new(4, HEAD_DIM, NUM_KV_HEADS, NUM_LAYERS, QuantNormMode::MaxNorm);
    let (k, v) = make_kv(4, 3.0);
    let q = make_q(4);
    let result = cache.prefill(LAYER, &k, &v, &q).unwrap();
    assert!(result.logit_bias.is_none(), "PQO4 should have no logit_bias");
}
...
fn tq3_decode_returns_logit_bias() {
    ...
    let q_dec = make_q(1);
    let config = attend_config();
    let output = cache.decode(LAYER, &k_dec, &v_dec, &q_dec, &config).unwrap();
...
fn create_tq3_cache() -> Box<dyn CompressedKVCache> {
    Box::new(TqCache::new(3, HEAD_DIM, NUM_KV_HEADS, NUM_LAYERS, QuantNormMode::MaxNorm))
}

fn create_tq4_cache() -> Box<dyn CompressedKVCache> {
    Box::new(TqCache::new(4, HEAD_DIM, NUM_KV_HEADS, NUM_LAYERS, QuantNormMode::MaxNorm))
}
